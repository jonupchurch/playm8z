...
const DEFAULT_IMAGE = {
  backgroundImage: "url(/admin-announcement-header.png)",
  backgroundSize: "cover",
  backgroundPosition: "center",
};

describe("newsCoverStyle", () => {
  it("renders a real uploaded image URL as-is", () => {
  ...
  it("falls back to the default header image when there is no cover (e.g. patch notes)", () => {
    expect(newsCoverStyle(null)).toEqual(DEFAULT_IMAGE);
  });

  it("falls back to the default header image for a legacy gradient cover (not a real image)", () => {
