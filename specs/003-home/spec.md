...
- **FR-005**: Each listing card in the feed MUST show the host's name and avatar, an approximate online indicator, a relative post age, region, game, title, a short blurb, its vibe tag, and its current open/total seat count.
...
- The green "online" indicator on each listing card approximates presence via the host's most recent activity, rather than a dedicated real-time presence system — the exact staleness window (e.g., a few minutes) is an implementation detail, not fixed here.
