...
- What happens with Report? → **Amended 2026-07-12**: no longer deferred — Notifications + Report modal (`012-notifications-and-report-modal`) provides the canonical report flow this feature was waiting on; the Report action opens that feature's modal against the listing (or, per-question, a specific Q&A entry). Share (a trivial copy-link/native-share action) stays in scope.
...
- **FR-014**: System MUST offer a "Share" action (e.g., copying the listing's URL) and a "Save" toggle creating/removing a `SavedListing` row for the viewing user.
...
- **FR-019** *(added 2026-07-12, see Assumptions)*: System MUST offer a "Report" action opening Notifications + Report modal's (`012`) canonical report flow against the listing (or a specific Q&A entry), requiring the same authentication + email-verification gate as every other write action here.
...
- **Report is in scope** (amended 2026-07-12): Notifications + Report modal (`012-notifications-and-report-modal`) provides the canonical report flow this feature was waiting on — this feature's Report action opens that modal (FR-019) rather than being excluded.
